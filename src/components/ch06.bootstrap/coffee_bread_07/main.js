...
function App() {
    ...
    */
    ...
    const CategoryChanged = (changedCategory) => {
        setFilterCategory(changedCategory); //선택된 카테고리 이름이 변경되었습니다.

        //필터링이 이루어 지면, 정렬 함수를 다시 실행 시켜 화면을 갱신시킵니다.
        Ordering(orderInfo);
    };

    //  필드 검색을 사용하여 필터링을 수행할지 말지를 결정하는 부을 타입의 변수
    const isFilteringNeeded = filterCategory && filterCategory !== 'all';

    // 삼항 연산자를 사용하여 선택한 카테고리와 동일한 품목들만 필터링 합니다.
    const filteredProducts = isFilteringNeeded ? products.filter((item) => item.category === filterCategory)
        : products;
